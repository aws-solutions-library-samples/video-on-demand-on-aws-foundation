...
> **Note (v2):** This solution has **no solution-authored Lambda functions** — every workflow step
> is a native AWS Step Functions service integration, and the only `AWS::Lambda::Function` in the
> synthesized template is the CDK-managed S3 bucket-notifications handler, which uses inline
> (`ZipFile`) code rather than an S3-backed asset. Consequently there are no asset-backed Lambdas
> and no `*AssetParameters` to rewrite, and **the helper runs as a no-op** (it logs that the
> template was left unchanged). The rewrite logic documented below is retained, guarded, so the
> build continues to work if an asset-backed Lambda is ever reintroduced.
